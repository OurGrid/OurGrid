...
public class BrokerWaitForJobCommand extends AbstractCommand<BrokerSyncApplicationClient> {

	public BrokerWaitForJobCommand( BrokerSyncApplicationClient componentClient ) {
	...
	protected void execute(String[] params) throws Exception {
		if (isComponentStarted()) {
			...
			final int jodID = Integer.parseInt( params[0] );
			System.out.println( BrokerUIMessages.getWaitForJobMessage( jodID ) );
			
			getComponentClient().waitForJob( jodID );
		} else {
