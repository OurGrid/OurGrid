...
public class RelativeCPUAccountingStrategy implements AccountingStrategy {
	...
	public RelativeCPUAccountingStrategy(String localPeerDN) {
	...
	public Map<String, Double> evaluate(Map<String, List<GridProcess>> processes) {
		...
		for (String providerDN : processes.keySet()) {
			...
			if (providerDN.equals(localPeerDN)) {
		...
		double totalAverage = receivedFavoursCount == 0 ? 0 : totalCpuTime / receivedFavoursCount;
