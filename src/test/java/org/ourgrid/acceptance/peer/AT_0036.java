...
public class AT_0036 extends PeerAcceptanceTestCase {
	...
	/**
	* Verifies if the Peer request update, pause and resume, done after the request finish.
    */
	@ReqTest(test="AT-0036", reqs="REQ014, REQ116, REQ117, REQ118")
	@Test public void test_AT_0036_ChangeStateAfterFinishRequest() throws Exception {
		...
		req_025_Util.changeWorkerStatusToAllocatedForBroker(component, lwpcOD, peerAcceptanceUtil.getRemoteWorkerManagementClientDeployment(), 
		...
		req_014_Util.finishRequestWithRemoteWorkers(component, peerAcceptanceUtil.getLocalWorkerProviderProxy(), 
				lwpOD, broker1PubKey, lwpc1OID.getServiceID(), 
				requestSpec1, new TestStub(rwpID, remotePeer));
		resetActiveMocks();
	
		//A local consumer update a finished request - expect to log warn
		RequestSpecification unknownRequestSpec = new RequestSpecification(0, new JobSpecification("label"), 1, "os = windows", 5, 0, 0);
		loggerMock.warn("The consumer ["+ lwpc1OID.getServiceID() +"] updated the unknown request ["+ unknownRequestSpec.getRequestId() +"]. " +
		...
		lwp.updateRequest(unknownRequestSpec);
		
		EasyMock.verify(loggerMock);
		EasyMock.reset(loggerMock);
		
		//A local consumer pause a finished request - expect to log warn
		loggerMock.warn("The consumer ["+ lwpc1OID.getServiceID() +"] paused the unknown request ["+ unknownRequestSpec.getRequestId() +"]. This message was ignored.");
		EasyMock.replay(loggerMock);
		
		AcceptanceTestUtil.setExecutionContext(component, lwpOD, lwpc1OID);
		lwp.pauseRequest(unknownRequestSpec.getRequestId());
		
		EasyMock.verify(loggerMock);
		EasyMock.reset(loggerMock);

		//A local consumer resume a finished request - expect to log warn
		loggerMock.warn("The consumer ["+ lwpc1OID.getServiceID() +"] resumed the unknown request ["+ unknownRequestSpec.getRequestId() +"]. This message was ignored.");
		EasyMock.replay(loggerMock);
		
		AcceptanceTestUtil.setExecutionContext(component, lwpOD, lwpc1OID);
		lwp.resumeRequest(unknownRequestSpec.getRequestId());
		
		EasyMock.verify(loggerMock);
	}
	
	@Category(JDLCompliantTest.class)
	@Test public void test_AT_0036_ChangeStateAfterFinishRequestWithJDL() throws Exception {
		//Create an user account
		XMPPAccount user1 = req_101_Util.createLocalUser("user011_1", "server011", "011011");
		...
		//A local consumer update a finished request - expect to log warn
		RequestSpecification unknownRequestSpec = new RequestSpecification(0, new JobSpecification("label"), 1, buildRequirements(null, null, "==", "windows"), 5, 0, 0);
		loggerMock.warn("The consumer ["+ lwpc1OID.getServiceID() +"] updated the unknown request ["+ unknownRequestSpec.getRequestId() +"]. " +
				"This message was ignored.");
		EasyMock.replay(loggerMock);
